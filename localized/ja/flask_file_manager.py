...
すべてのバックエンド、HTML、CSS、JavaScript はこのファイルに含まれています。
実行前に requirements.txt の依存関係をインストールしてください。
...
def get_share_or_404(token: str) -> Dict[str, Any]:
    ...
    if not share:
        abort(404, description="共有が存在しないか取り消されています")

    root = safe_path(share.get("path", ""))
    if not root.exists():
        abort(404, description="共有元ファイルが存在しません")
...
def resolve_shared_path(share: Dict[str, Any], rel_path: str | None = "") -> Path:
    ...
    if not is_within(target, root):
        abort(403, description="共有ディレクトリ外へのアクセスは許可されていません")
...
def media_type_for(path: Path) -> Optional[str]:
    """
    オンライン再生の可否は拡張子で厳密に判定します。
    VIDEO_EXTENSIONS / AUDIO_EXTENSIONS に登録された拡張子のみ再生操作を表示します。
    """
    if not path.is_file():
        return None
...
def is_text_file(path: Path) -> bool:
    """
    オンライン編集の可否は拡張子で厳密に判定します。
    TEXT_EXTENSIONS に登録された拡張子のみ編集操作を表示します。
    """
    if not path.is_file():
        return False
...
def list_dir_payload(current: Path) -> Dict[str, Any]:
    if not current.exists():
        return {"ok": False, "error": "パスが存在しません"}
...
def partial_response(path: Path):
    """
    video/audio 要素でシークできるよう Range リクエストをサポートします。
    """
...
def make_7z_archive(paths: list[Path], base_dir: Path, archive_name: str) -> Path:
    ...
    try:
        ...
        abort(400, description="無効なアーカイブ名")

    # preset=9 py7zr が提供する LZMA2 の最高プリセットです。
...
def api_mkdir():
    ...
    if not parent.exists() or not parent.is_dir():
        return jsonify({"ok": False, "error": "親ディレクトリが存在しません"}), 404

    if not name:
        return jsonify({"ok": False, "error": "フォルダー名は空にできません"}), 400

    if "/" in name or "\\" in name or name in {".", ".."}:
        return jsonify({"ok": False, "error": "無効なフォルダー名"}), 400

    parent_rel = rel_from_root(parent) if parent != STORAGE_ROOT else ""
    target = safe_path(f"{parent_rel}/{name}" if parent_rel else name)

    if target.exists():
        return jsonify({"ok": False, "error": "フォルダーは既に存在します"}), 409
...
def api_upload():
    ...
    if not target_dir.exists() or not target_dir.is_dir():
        return jsonify({"ok": False, "error": "アップロード先フォルダーが存在しません"}), 404
...
def api_download():
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "ファイルが存在しません"}), 404
...
def api_media():
    ...
    if media_type_for(path) not in {"video", "audio"}:
        return jsonify({"ok": False, "error": "この拡張子はオンライン再生に対応していません"}), 400
...
def api_text():
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "このファイル形式はオンライン編集に対応していません"}), 400
...
def api_text_save():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"未対応のエンコーディング: {encoding}"}), 400
    except UnicodeEncodeError:
        return jsonify({"ok": False, "error": f"現在の内容は {encoding} エンコーディングで保存できません"}), 400
...
def api_delete():
    ...
    if path == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "ルートディレクトリは削除できません"}), 400

    if not path.exists():
        return jsonify({"ok": False, "error": "パスが存在しません"}), 404
...
def api_move():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "ルートディレクトリは移動できません"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "移動元パスが存在しません"}), 404

    if dst.exists():
        return jsonify({"ok": False, "error": "移動先パスは既に存在します"}), 409

    dst.parent.mkdir(parents=True, exist_ok=True)

    if src.is_dir() and is_within(dst, src):
        return jsonify({"ok": False, "error": "フォルダーを自身の内部へ移動できません"}), 400
...
def api_rename():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "ルートディレクトリは名前変更できません"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "パスが存在しません"}), 404

    if not new_name or "/" in new_name or "\\" in new_name or new_name in {".", ".."}:
        return jsonify({"ok": False, "error": "無効な名前"}), 400
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "無効なパス"}), 400

    if dst.exists():
        return jsonify({"ok": False, "error": "移動先の名前は既に存在します"}), 409
...
def api_create_share():
    ...
    if not path.exists():
        return jsonify({"ok": False, "error": "共有パスが存在しません"}), 404

    token = secrets.token_urlsafe(24)
    shares = load_shares()
    shares[token] = {
        "path": rel_from_root(path) if path != STORAGE_ROOT else "",
        "name": path.name if path != STORAGE_ROOT else "ルート",
        "type": "dir" if path.is_dir() else "file",
...
def shared_text(token: str):
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "このファイル形式はオンライン表示に対応していません"}), 400
...
def api_create_archive():
    ...
    if not rel_paths:
        return jsonify({"ok": False, "error": "少なくとも1つのファイルまたはフォルダーを選択してください"}), 400

    resolved = []
    for rel in rel_paths:
        p = safe_path(rel)
        if not p.exists():
            return jsonify({"ok": False, "error": f"パスが存在しません: {rel}"}), 404
        resolved.append(p)
    ...
    if len(resolved) == 1:
        ...
        prefix = "selected"
        # 複数選択の圧縮では現在のディレクトリをアーカイブ基準にします；cwd が指定されない場合は storage ルートを使用します。
...
def api_download_archive(name: str):
    if "/" in name or "\\" in name or not name.endswith(".7z"):
        return jsonify({"ok": False, "error": "無効なアーカイブ名"}), 400
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "アーカイブが存在しないか削除されています"}), 404
...
if __name__ == "__main__":
    print("既定のユーザー名：admin")
    print("既定のパスワード：admin123")
    print("FM_USERNAME / FM_PASSWORD で変更できます。")
