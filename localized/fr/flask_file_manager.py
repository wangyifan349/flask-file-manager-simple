...
Tout le backend, le HTML, le CSS et le JavaScript sont contenus dans ce fichier.
Installez les dépendances de requirements.txt avant l’exécution :
...
def get_share_or_404(token: str) -> Dict[str, Any]:
    ...
    if not share:
        abort(404, description="Le partage n’existe pas ou a été révoqué")

    root = safe_path(share.get("path", ""))
    if not root.exists():
        abort(404, description="Le fichier source partagé n’existe pas")
...
def resolve_shared_path(share: Dict[str, Any], rel_path: str | None = "") -> Path:
    root = share["root_abs"]
    if root.is_file():
        # Pour les partages de fichiers, seul le fichier partagé lui-même est accessible.
        target = root
    else:
        rel_path = (rel_path or "").strip().lstrip("/\\")
    ...
    if not is_within(target, root):
        abort(403, description="L’accès hors du dossier partagé n’est pas autorisé")
...
def media_type_for(path: Path) -> Optional[str]:
    """
    Détermine strictement la lecture en ligne par extension de fichier.
    Seules les extensions enregistrées dans VIDEO_EXTENSIONS / AUDIO_EXTENSIONS affichent les actions de lecture.
    """
    if not path.is_file():
        return None
...
def is_text_file(path: Path) -> bool:
    """
    Détermine strictement l’édition en ligne par extension de fichier.
    Seules les extensions enregistrées dans TEXT_EXTENSIONS affichent les actions d’édition.
    """
    if not path.is_file():
        return False
...
def partial_response(path: Path):
    """
    Prend en charge les requêtes Range pour permettre la recherche dans les éléments vidéo/audio.
    """
...
def make_7z_archive(paths: list[Path], base_dir: Path, archive_name: str) -> Path:
    ...
    try:
        ...
        abort(400, description="Nom d’archive invalide")

    # preset=9 est le plus haut préréglage LZMA2 exposé par py7zr.
...
def api_mkdir():
    ...
    if not parent.exists() or not parent.is_dir():
        return jsonify({"ok": False, "error": "Le dossier parent n’existe pas"}), 404

    if not name:
        return jsonify({"ok": False, "error": "Le nom du dossier ne peut pas être vide"}), 400
    ...
    if target.exists():
        return jsonify({"ok": False, "error": "Le dossier existe déjà"}), 409
...
def api_upload():
    ...
    if not target_dir.exists() or not target_dir.is_dir():
        return jsonify({"ok": False, "error": "Le dossier cible de téléversement n’existe pas"}), 404
...
def api_download():
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "Le fichier n’existe pas"}), 404
...
def api_media():
    ...
    if media_type_for(path) not in {"video", "audio"}:
        return jsonify({"ok": False, "error": "Cette extension ne prend pas en charge la lecture en ligne"}), 400
...
def api_text():
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "Ce type de fichier ne prend pas en charge l’édition en ligne"}), 400
...
def api_text_save():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Encodage non pris en charge : {encoding}"}), 400
    except UnicodeEncodeError:
        return jsonify({"ok": False, "error": f"Le contenu actuel ne peut pas être enregistré avec {encoding} encodage"}), 400
...
def api_delete():
    ...
    if path == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Impossible de supprimer le dossier racine"}), 400

    if not path.exists():
        return jsonify({"ok": False, "error": "Le chemin n’existe pas"}), 404
...
def api_move():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Impossible de déplacer le dossier racine"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "Le chemin source n’existe pas"}), 404

    if dst.exists():
        return jsonify({"ok": False, "error": "Le chemin cible existe déjà"}), 409

    dst.parent.mkdir(parents=True, exist_ok=True)

    if src.is_dir() and is_within(dst, src):
        return jsonify({"ok": False, "error": "Impossible de déplacer un dossier dans lui-même"}), 400
...
def api_rename():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Impossible de renommer le dossier racine"}), 400
    ...
    if dst.exists():
        return jsonify({"ok": False, "error": "Le nom cible existe déjà"}), 409
...
def api_create_share():
    ...
    if not path.exists():
        return jsonify({"ok": False, "error": "Le chemin de partage n’existe pas"}), 404

    token = secrets.token_urlsafe(24)
    shares = load_shares()
    shares[token] = {
        "path": rel_from_root(path) if path != STORAGE_ROOT else "",
        "name": path.name if path != STORAGE_ROOT else "Racine",
        "type": "dir" if path.is_dir() else "file",
...
def shared_text(token: str):
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "Ce type de fichier ne prend pas en charge l’affichage en ligne"}), 400
...
def api_create_archive():
    ...
    if not rel_paths:
        return jsonify({"ok": False, "error": "Sélectionnez au moins un fichier ou dossier"}), 400

    resolved = []
    for rel in rel_paths:
        p = safe_path(rel)
        if not p.exists():
            return jsonify({"ok": False, "error": f"Le chemin n’existe pas : {rel}"}), 404
        resolved.append(p)
    ...
    if len(resolved) == 1:
        ...
        prefix = "selected"
        # Pour les archives multi-sélection, le dossier courant sert de base；Utilise la racine storage lorsque cwd n’est pas fourni.
...
def api_download_archive(name: str):
    if "/" in name or "\\" in name or not name.endswith(".7z"):
        return jsonify({"ok": False, "error": "Nom d’archive invalide"}), 400
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "L’archive n’existe pas ou a été nettoyée"}), 404
