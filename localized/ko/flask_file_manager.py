...
모든 백엔드, HTML, CSS, JavaScript가 이 파일에 포함되어 있습니다.
실행 전에 requirements.txt의 의존성을 설치하세요.
...
def get_share_or_404(token: str) -> Dict[str, Any]:
    ...
    if not share:
        abort(404, description="공유가 없거나 취소되었습니다")

    root = safe_path(share.get("path", ""))
    if not root.exists():
        abort(404, description="공유 원본 파일이 없습니다")
...
def resolve_shared_path(share: Dict[str, Any], rel_path: str | None = "") -> Path:
    root = share["root_abs"]
    if root.is_file():
        # 파일 공유에서는 공유된 파일 자체만 접근할 수 있습니다.
        target = root
    else:
        rel_path = (rel_path or "").strip().lstrip("/\\")
    ...
    if not is_within(target, root):
        abort(403, description="공유 디렉터리 밖에는 접근할 수 없습니다")
...
def media_type_for(path: Path) -> Optional[str]:
    """
    온라인 재생 지원은 확장자로 엄격하게 판단합니다.
    VIDEO_EXTENSIONS / AUDIO_EXTENSIONS에 등록된 확장자만 재생 작업을 표시합니다.
    """
    if not path.is_file():
        return None
...
def is_text_file(path: Path) -> bool:
    """
    온라인 편집 지원은 확장자로 엄격하게 판단합니다.
    TEXT_EXTENSIONS에 등록된 확장자만 편집 작업을 표시합니다.
    """
    if not path.is_file():
        return False
...
def list_dir_payload(current: Path) -> Dict[str, Any]:
    ...
    if not current.is_dir():
        return {"ok": False, "error": "디렉터리가 아닙니다"}
...
def partial_response(path: Path):
    """
    비디오/오디오 요소의 탐색을 위해 Range 요청을 지원합니다.
    """
...
def make_7z_archive(paths: list[Path], base_dir: Path, archive_name: str) -> Path:
    ...
    try:
        ...
        abort(400, description="잘못된 압축 파일 이름")

    # preset=9 py7zr가 제공하는 가장 높은 LZMA2 프리셋입니다.
...
def api_mkdir():
    ...
    if not parent.exists() or not parent.is_dir():
        return jsonify({"ok": False, "error": "상위 디렉터리가 없습니다"}), 404

    if not name:
        return jsonify({"ok": False, "error": "폴더 이름은 비워둘 수 없습니다"}), 400

    if "/" in name or "\\" in name or name in {".", ".."}:
        return jsonify({"ok": False, "error": "잘못된 폴더 이름"}), 400

    parent_rel = rel_from_root(parent) if parent != STORAGE_ROOT else ""
    target = safe_path(f"{parent_rel}/{name}" if parent_rel else name)

    if target.exists():
        return jsonify({"ok": False, "error": "폴더가 이미 있습니다"}), 409
...
def api_upload():
    ...
    if not target_dir.exists() or not target_dir.is_dir():
        return jsonify({"ok": False, "error": "업로드 대상 폴더가 없습니다"}), 404

    files = request.files.getlist("files")
    if not files:
        return jsonify({"ok": False, "error": "선택한 파일이 없습니다"}), 400
...
def api_media():
    ...
    if media_type_for(path) not in {"video", "audio"}:
        return jsonify({"ok": False, "error": "이 확장자는 온라인 재생을 지원하지 않습니다"}), 400
...
def api_text():
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "이 파일 형식은 온라인 편집을 지원하지 않습니다"}), 400
...
def api_text_save():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"지원하지 않는 인코딩: {encoding}"}), 400
    except UnicodeEncodeError:
        return jsonify({"ok": False, "error": f"현재 내용은 {encoding} 인코딩으로 저장할 수 없습니다"}), 400
...
def api_delete():
    ...
    if path == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "루트 디렉터리는 삭제할 수 없습니다"}), 400

    if not path.exists():
        return jsonify({"ok": False, "error": "경로가 없습니다"}), 404
...
def api_move():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "루트 디렉터리는 이동할 수 없습니다"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "원본 경로가 없습니다"}), 404

    if dst.exists():
        return jsonify({"ok": False, "error": "대상 경로가 이미 있습니다"}), 409

    dst.parent.mkdir(parents=True, exist_ok=True)

    if src.is_dir() and is_within(dst, src):
        return jsonify({"ok": False, "error": "폴더를 자기 내부로 이동할 수 없습니다"}), 400
...
def api_rename():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "루트 디렉터리는 이름을 바꿀 수 없습니다"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "경로가 없습니다"}), 404

    if not new_name or "/" in new_name or "\\" in new_name or new_name in {".", ".."}:
        return jsonify({"ok": False, "error": "잘못된 이름"}), 400
    ...
    if dst.exists():
        return jsonify({"ok": False, "error": "대상 이름이 이미 있습니다"}), 409
...
def api_create_share():
    ...
    if not path.exists():
        return jsonify({"ok": False, "error": "공유 경로가 없습니다"}), 404

    token = secrets.token_urlsafe(24)
    shares = load_shares()
    shares[token] = {
        "path": rel_from_root(path) if path != STORAGE_ROOT else "",
        "name": path.name if path != STORAGE_ROOT else "루트",
        "type": "dir" if path.is_dir() else "file",
...
def shared_text(token: str):
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "이 파일 형식은 온라인 보기를 지원하지 않습니다"}), 400
...
def api_create_archive():
    ...
    if not rel_paths:
        return jsonify({"ok": False, "error": "파일 또는 폴더를 하나 이상 선택하세요"}), 400

    resolved = []
    for rel in rel_paths:
        p = safe_path(rel)
        if not p.exists():
            return jsonify({"ok": False, "error": f"경로가 없습니다: {rel}"}), 404
        resolved.append(p)
    ...
    if len(resolved) == 1:
        ...
        prefix = "selected"
        # 다중 선택 압축은 현재 디렉터리를 기준으로 합니다；cwd가 없으면 storage 루트를 사용합니다.
...
def api_download_archive(name: str):
    if "/" in name or "\\" in name or not name.endswith(".7z"):
        return jsonify({"ok": False, "error": "잘못된 압축 파일 이름"}), 400
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "압축 파일이 없거나 정리되었습니다"}), 404
...
if __name__ == "__main__":
    print("기본 사용자 이름: admin")
    print("기본 비밀번호: admin123")
    print("FM_USERNAME / FM_PASSWORD로 변경할 수 있습니다.")
