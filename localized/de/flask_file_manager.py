...
Backend, HTML, CSS und JavaScript befinden sich vollständig in dieser Datei.
Installiere vor dem Start die Abhängigkeiten aus requirements.txt:
...
def safe_path(rel_path: str | None = "") -> Path:
    ...
    try:
        ...
        abort(400, description="Ungültiger Pfad")
...
def get_share_or_404(token: str) -> Dict[str, Any]:
    ...
    if not share:
        abort(404, description="Freigabe existiert nicht oder wurde aufgehoben")

    root = safe_path(share.get("path", ""))
    if not root.exists():
        abort(404, description="Quelldatei der Freigabe existiert nicht")
...
def resolve_shared_path(share: Dict[str, Any], rel_path: str | None = "") -> Path:
    root = share["root_abs"]
    if root.is_file():
        # Bei Dateifreigaben darf nur die freigegebene Datei selbst aufgerufen werden.
        target = root
    else:
        rel_path = (rel_path or "").strip().lstrip("/\\")
    ...
    if not is_within(target, root):
        abort(403, description="Zugriff außerhalb des freigegebenen Ordners ist nicht erlaubt")
...
def media_type_for(path: Path) -> Optional[str]:
    """
    Online-Wiedergabe wird strikt anhand der Dateiendung bestimmt.
    Nur in VIDEO_EXTENSIONS / AUDIO_EXTENSIONS registrierte Endungen zeigen Wiedergabeaktionen.
    """
    if not path.is_file():
        return None
...
def is_text_file(path: Path) -> bool:
    """
    Online-Bearbeitung wird strikt anhand der Dateiendung bestimmt.
    Nur in TEXT_EXTENSIONS registrierte Endungen zeigen Bearbeitungsaktionen.
    """
    if not path.is_file():
        return False
...
def partial_response(path: Path):
    """
    Unterstützt Range-Anfragen, damit Video-/Audioelemente springen können.
    """
...
def make_7z_archive(paths: list[Path], base_dir: Path, archive_name: str) -> Path:
    ...
    try:
        ...
        abort(400, description="Ungültiger Archivname")

    # preset=9 ist das höchste von py7zr bereitgestellte LZMA2-Preset.
...
def api_mkdir():
    ...
    if not parent.exists() or not parent.is_dir():
        return jsonify({"ok": False, "error": "Übergeordneter Ordner existiert nicht"}), 404

    if not name:
        return jsonify({"ok": False, "error": "Ordnername darf nicht leer sein"}), 400
...
def api_upload():
    ...
    if not target_dir.exists() or not target_dir.is_dir():
        return jsonify({"ok": False, "error": "Upload-Zielordner existiert nicht"}), 404
...
def api_download():
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "Datei existiert nicht"}), 404
...
def api_media():
    ...
    if media_type_for(path) not in {"video", "audio"}:
        return jsonify({"ok": False, "error": "Diese Dateiendung unterstützt keine Online-Wiedergabe"}), 400
...
def api_text():
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "Dieser Dateityp unterstützt keine Online-Bearbeitung"}), 400
...
def api_text_save():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Nicht unterstützte Kodierung: {encoding}"}), 400
    except UnicodeEncodeError:
        return jsonify({"ok": False, "error": f"Der aktuelle Inhalt kann nicht gespeichert werden mit {encoding} Kodierung"}), 400
...
def api_delete():
    ...
    if path == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Root-Ordner kann nicht gelöscht werden"}), 400

    if not path.exists():
        return jsonify({"ok": False, "error": "Pfad existiert nicht"}), 404
...
def api_move():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Root-Ordner kann nicht verschoben werden"}), 400

    if not src.exists():
        return jsonify({"ok": False, "error": "Quellpfad existiert nicht"}), 404

    if dst.exists():
        return jsonify({"ok": False, "error": "Zielpfad existiert bereits"}), 409

    dst.parent.mkdir(parents=True, exist_ok=True)

    if src.is_dir() and is_within(dst, src):
        return jsonify({"ok": False, "error": "Ein Ordner kann nicht in sich selbst verschoben werden"}), 400
...
def api_rename():
    ...
    if src == STORAGE_ROOT:
        return jsonify({"ok": False, "error": "Root-Ordner kann nicht umbenannt werden"}), 400
    ...
    if dst.exists():
        return jsonify({"ok": False, "error": "Zielname existiert bereits"}), 409
...
def api_create_share():
    ...
    if not path.exists():
        return jsonify({"ok": False, "error": "Freigabepfad existiert nicht"}), 404

    token = secrets.token_urlsafe(24)
    shares = load_shares()
    shares[token] = {
        "path": rel_from_root(path) if path != STORAGE_ROOT else "",
        "name": path.name if path != STORAGE_ROOT else "Root",
        "type": "dir" if path.is_dir() else "file",
...
def shared_text(token: str):
    ...
    if not is_text_file(path):
        return jsonify({"ok": False, "error": "Dieser Dateityp unterstützt keine Online-Ansicht"}), 400
...
def api_create_archive():
    ...
    if not rel_paths:
        return jsonify({"ok": False, "error": "Mindestens eine Datei oder einen Ordner auswählen"}), 400

    resolved = []
    for rel in rel_paths:
        p = safe_path(rel)
        if not p.exists():
            return jsonify({"ok": False, "error": f"Pfad existiert nicht: {rel}"}), 404
        resolved.append(p)
    ...
    if len(resolved) == 1:
        ...
        prefix = "selected"
        # Bei Mehrfachauswahl-Archiven dient der aktuelle Ordner als Basis；Wenn cwd fehlt, wird das storage-Root verwendet.
...
def api_download_archive(name: str):
    if "/" in name or "\\" in name or not name.endswith(".7z"):
        return jsonify({"ok": False, "error": "Ungültiger Archivname"}), 400
    ...
    if not path.exists() or not path.is_file():
        return jsonify({"ok": False, "error": "Archiv existiert nicht oder wurde bereinigt"}), 404
